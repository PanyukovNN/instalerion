...
public class InstagramPostPublishingStrategy implements PublishingStrategy {

    private final Logger logger = LoggerFactory.getLogger(InstagramPostPublishingStrategy.class);
    ...
    private Post definePost(PublishingRequest request) {
        Post post = postService.findMostRecent(request.getProducingChannelId())
                .orElse(null);
    ...
    private void checkPost(PublishingRequest request, Post post) {
        if (post == null) {
            throw new RequestException(String.format(POST_NOT_FOUND_ERROR_MSG, request.getProducingChannelId()));
        }
